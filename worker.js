...
const ENDPOINTS = [
  ...
  { method: "POST", path: "/api/v1/alight-motion/send", category: "Alight Motion Premium", description: "AM Premium Send — mengirim request send melalui upstream Alight Motion Premium yang dikonfigurasi. Gunakan secret/variable upstream yang sah; jangan hardcode auth-link/token.", params: [{ name: "email", type: "string", required: true }] },
...
async function proxy(url, req, env, fields = {}) {
  ...
  const headers = { accept: "application/json,text/plain,*/*", "user-agent": "XTZYYY-Premium-API/4.3" };
...
async function handleApi(req, env, ctx) {
  ...
  if (path === "/api/v1/alight-motion/send" || path === "/api/v1/alight-motion/verif") {
    ...
    if (missing.length) return json({ status: false, creator: CREATOR, message: "Parameter wajib belum diisi", missing }, 400);
    const isVerify = path.endsWith("/verif");
    const target = isVerify ? (env.ALIGHT_VERIFY_URL || env.ALIGHT_PREM_URL) : (env.ALIGHT_SEND_URL || env.ALIGHT_PREM_URL);
    const u = normalizeUrl(target);
    if (!u && String(env.ALIGHT_MOCK_MODE || "true").toLowerCase() === "true") {
      return json({ status: true, creator: CREATOR, mode: "mock", message: `Alight Motion ${isVerify ? "verify" : "send"} berhasil diproses dalam TEST MODE`, data: { email: String(b.email).trim(), action: isVerify ? "verify" : "send", link: isVerify ? String(b.link).trim() : undefined, simulated: true } });
    }
    if (!u) return json({ status: false, creator: CREATOR, message: "Alight Motion upstream belum dikonfigurasi" }, 501);
    const headers = { "content-type": "application/json", accept: "application/json,text/plain,*/*", "user-agent": "XTZYYY-Premium-API/4.3" }; if (env.EXTERNAL_API_KEY) headers["x-apikey"] = env.EXTERNAL_API_KEY;
