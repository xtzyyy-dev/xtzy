(function(){
  ...
  async function globalStatus(){
    ...
    try{
      ...
      document.querySelectorAll('[data-endpoint-count]').forEach(el=>el.textContent=String(d.service?.endpointCount ?? '—'));
      document.querySelectorAll('[data-global-version]').forEach(el=>el.textContent=d.service?.version||'—');
      document.querySelectorAll('[data-public-url]').forEach(el=>{const u=d.service?.publicUrl||''; el.textContent=u||'Menunggu link HTTPS...'; el.href=u||'#'; if(u) el.target='_blank';});
      const metrics={
        '[data-total-users]':d.service?.totalUsers,
        '[data-total-requests]':d.service?.totalRequests,
        '[data-success-requests]':d.service?.successfulRequests,
        '[data-failed-requests]':d.service?.failedRequests,
        '[data-total-orders]':d.service?.totalOrders,
        '[data-active-requests]':d.service?.activeRequests,
        '[data-memory-usage]':(d.service?.memoryUsageMb??0)+' MB',
        '[data-node-version]':d.service?.nodeVersion||'—'
      };
      Object.entries(metrics).forEach(([sel,val])=>document.querySelectorAll(sel).forEach(el=>el.textContent=Number(val??0).toLocaleString('id-ID')));
      const top=document.querySelector('#top-endpoints');
      if(top){
        const rows=d.service?.topEndpoints||[];
        top.innerHTML=rows.length?rows.map((x,i)=>`<div class="info-card"><div class="label">#${i+1}</div><strong>${String(x.path)}</strong><p>${Number(x.hits||0).toLocaleString('id-ID')} requests</p></div>`).join(''):'<div class="info-card"><strong>Belum ada traffic</strong><p>Request endpoint akan muncul di sini.</p></div>';
      }
